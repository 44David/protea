...
export async function activate(context: vscode.ExtensionContext) {
	...
	const disposable = vscode.commands.registerCommand('protea.protea-search', async () => {
		...
		try {
			...
			localModelNames = response.models.map((model: any) => model.name)
		} catch (error) {
			console.error("Failed to fetch local Ollama modesl:", error);
		...
		const panel = vscode.window.createWebviewPanel (
			...
			{ 
				enableScripts: true, 
				localResourceRoots: [vscode.Uri.joinPath(context.extensionUri, 'media')] // Security
			}

		);

		panel.webview.html = getWebviewcontent(localModelNames)
